...
$(document).ready(function() {
  ...
  $(document).on("click", "#pause:not(.disabled)", function() {
    if ($(this).hasClass("resume")) {
      Fr.voice.resume();
        if($('body').hasClass('ar_body')){
        $(this).replaceWith('<a class="button one" id="pause">توقف</a>');
        } else{
        ...
        Fr.voice.pause();
        if($('body').hasClass('ar_body')){
        $(this).replaceWith('<a class="button one resume" id="pause">إعادة التسجيل</a>');
        }else{
...
var type = document.querySelector('.type').id; 
      ...
      formData.append('type', type);      formData.append('audioid', audioid);
